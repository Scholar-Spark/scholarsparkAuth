...
# Function to generate service templates
generate_service_templates() {
    SERVICE_NAME=$(get_project_name)
    
    # Create Helm chart directory structure
    mkdir -p "helm/${SERVICE_NAME}/templates"
    
    # Generate Chart.yaml
    cat > "helm/${SERVICE_NAME}/Chart.yaml" << EOF
apiVersion: v2
name: ${SERVICE_NAME}
...
EOF
    ...
    cat > "helm/${SERVICE_NAME}/values.yaml" << EOF
image:
...
EOF
    ...
    cat > "helm/${SERVICE_NAME}/templates/deployment.yaml" << EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {{ .Release.Name }}
...
EOF
    ...
    cat > "helm/${SERVICE_NAME}/templates/service.yaml" << EOF
apiVersion: v1
kind: Service
metadata:
  name: {{ .Release.Name }}
...
EOF
    ...
    cat > "helm/${SERVICE_NAME}/templates/database.yaml" << EOF
apiVersion: apps/v1
kind: Deployment
metadata:
...
validate_templates() {
    ...
    if ! helm lint "helm/${SERVICE_NAME}"; then
